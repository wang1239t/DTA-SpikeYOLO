...
class Model(nn.Module):
    ...
    def _new(self, cfg: str, task=None, model=None, verbose=True):
        ...
        """
        cfg_dict = yaml_model_load(cfg) # 加载模型字典
        self.cfg = cfg
        self.task = task or guess_model_task(cfg_dict)
        self.model = (model or self._smart_load('model'))(cfg_dict, verbose=verbose and RANK == -1)  # build model,这是一个函数调用，调用model or self._smart_load('model')返回的对象
    ...
    def val(self, validator=None, **kwargs):
        ...
        """
        # custom = {'rect': True}  # method defaults 这里默认会将rect改为true
        custom = {}  # method defaults 这里默认会将rect改为true
