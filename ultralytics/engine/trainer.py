...
class BaseTrainer:
    ...
    def _do_train(self, world_size=1):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                # pltimage(batch, save_dir='./save_batch_sod/')
...
def pltimage(batch, save_dir='./save_batch/'):
    ...
    # (4,w,h,3)
    im = batch['img']
    print(f'处理后图片前最大值{torch.max(im)}')
    # 如果知道确切的最小值和最大值
    min_val = im.min()
    max_val = im.max()
    im = (im - min_val) / (max_val - min_val)  # (8,4,3,320,320)tensor
    bboxes_all = batch['bboxes']
    cls_ids_all = batch['cls']
    batch_idx = batch['batch_idx']
    for i in range(im.shape[0]):
        img = im[i]
        im_file_path = batch['im_file'][i]
        filename = os.path.basename(im_file_path)  # 获取文件名（含扩展名）
        desired_part = os.path.splitext(filename)[0]
        mask = (batch_idx == i)  # 生成布尔掩码
        bboxes = bboxes_all[mask].numpy()
        cls_ids = cls_ids_all[mask].numpy()
        for j in range(img.shape[0]):
            # 将通道赋值给 RGB 图像的对应通道
            image = img[j].numpy()
            additional_channel = np.zeros((1, 640, 640))  # 创建形状为 (1, 640, 640) 的全零数组
            rgb_image = np.concatenate((image, additional_channel), axis=0)
            ...
            # 保存图形而不是显示
            save_path = os.path.join(save_dir, f'output_{desired_part}_image_{i}_with_bboxes_frame_{j + 1}_channel.png')
