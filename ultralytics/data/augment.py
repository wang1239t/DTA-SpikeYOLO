...
class Compose:
    ...
    def __call__(self, data):
        ...
        for t in self.transforms:  # 7大组，第一大组有3小组，包括mosaic,copypaste,randomperspective(递归执行)
            data = t(data)  # copy
            # print(t)
        return data
...
class RandomPerspective:
    ...
    def __call__(self, labels):
        ...
        if len(img.shape) == 4:
            self.size = img.shape[2] + border[1] * 2, img.shape[1] + border[0] * 2  # w, h
        else:
        ...
        labels['img'] = img
        if len(img.shape) == 4:
            labels['resized_shape'] = img.shape[1:3]  # 1111修改
        else:
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        new_shape = labels.pop('rect_shape', self.new_shape)  # 这行代码的作用是从字典 labels 中安全地获取并移除一个特定键的值，同时处理键不存在的情况。
        # new_shape = [320,320]  #111111重新定义new_shape
        if isinstance(new_shape, int):
            new_shape = (new_shape, new_shape)

        # Scale ratio (new / old)  #304,240  → 320  253 → 320  256
        r = min(new_shape[0] / shape[0], new_shape[1] / shape[1])
        if not self.scaleup:  # only scale down, do not scale up (for better val mAP)
            r = min(r, 1.0)
        # 变换前检验标签
        # pltimage_before(labels, save_dir='./save_letter_before/')  # 标签错,(4,241,320,3)
        ...
        if shape[::-1] != new_unpad:  # resize # shape[::-1]对shape进行翻转
            if len(img.shape) == 4:  # 处理动态数据集:
                T, H, W, C = img.shape
                img_new = np.zeros([T, new_unpad[1], new_unpad[0], C])  # 进行缩放
                for i in range(T):
                    # img_new[i] = cv2.resize(img[i], (new_unpad[1], new_shape[0]), interpolation=cv2.INTER_LINEAR)
                    # 原版，並沒有走這裡，可能训练的时候有其他数据增强，输入数据的shape已经是[320,253]了，因此不需要走这个支路
                    # 如果推理时候输入shape是[304,240]，要走这个支路，所以这里要修改
        ...
        if len(img.shape) == 4:  # 处理动态数据集:   难道是直接reshape的？
            size = max(new_shape[1], new_shape[0])
            T, H, W, C = img.shape
            img_final = np.zeros(
                [T, new_unpad[1] + top + bottom, new_unpad[0] + left + right, C])  # 进行填充 这里进行修改，根据new_unpad而不是正方形
            for i in range(T):
            ...
            img = cv2.copyMakeBorder(img, top, bottom, left, right, cv2.BORDER_CONSTANT,
                                     # 将函数扩展为可以被32整除的格式(根据参数可能拓展为矩形) 这里应该也是hwc
                                     value=(0, 0, 0))  # add border
        ...
        if len(labels):
            ...
            # pltimage(labels, save_dir='./save_letter/')  # 标签错
            return labels
        else:
            return img  # 返回的仍然是hwc

    def _update_labels(self, labels, ratio, padw, padh):
        """Update labels."""
        labels['instances'].convert_bbox(format='xyxy')
        if len(labels['img'].shape) == 4:
            labels['instances'].denormalize(*labels['img'].shape[1:3][::-1])  # 11111在这里会对标签进行缩放!!
        else:
...
def pltimage(labels, save_dir='./save_letter/'):
    ...
    # (4,w,h,3)
    im = labels['img'].astype(np.uint8)
    T, H, W, C = im.shape
    instances = labels['instances']
    bboxes = instances.bboxes
    cls_ids = labels['cls']
    im_file_path = labels['im_file']
    filename = os.path.basename(im_file_path)  # 获取文件名（含扩展名）
    desired_part = os.path.splitext(filename)[0]
    ...
    for i in range(im.shape[0]):
        ...
        # 保存图形而不是显示
        save_path = os.path.join(save_dir, f'output_image_{desired_part}_with_bboxes_frame_{i + 1}_channel.png')
        plt.savefig(save_path, bbox_inches='tight')
        plt.close()


def pltimage_before(labels, save_dir='./save_letter_before/'):
    os.makedirs(save_dir, exist_ok=True)
    classes = {
        0: 'car',
        1: 'two-wheel',
    ...
    for i in range(im.shape[0]):
        ...
        for bbox, cls_id in zip(bboxes, cls_ids):
            ...
            # 确保坐标不越界
            xmin, xmax = np.clip([x_min, x_max], 0, W)
            ymin, ymax = np.clip([y_min, y_max], 0, H)
