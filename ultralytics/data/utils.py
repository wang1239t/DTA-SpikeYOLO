...
def verify_image_label(args):
    ...
    im_file, lb_file, prefix, _, num_cls, *_ = args  # 解包参数

    # 初始化统计变量和结果占位
    nm, nf, ne, nc, msg = 0, 0, 0, 0, ''
    lb = np.zeros((0, 5), dtype=np.float32)  # 默认空标签
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                ...
                lb = np.array(lb, dtype=np.float32)  # 可能抛出 ValueError
        ...
        nc = 1  # 标记为损坏
        msg = f'{prefix}ERROR ❌ {lb_file}: {e}'  # 记录到返回消息

    # 返回结果（保持结构一致）
    return im_file, lb, nm, nf, ne, nc, msg

# def verify_image_label(args):
#     """Verify one image-label pair (仅路径级验证，不加载npy数据)"""
...
#             nc = 1
#             msg = f'{prefix}ERROR ❌ 时空数据文件丢失: {im_file}'
#             return im_file, None, None, None, None, nm, nf, ne, nc, msg
#
#         # 基础验证2：快速加载头信息（可选）
#         try:
#             with open(im_file, 'rb') as f:
#                 np.lib.format.read_magic(f)
#             # 可添加更多元数据检查（如shape dtype）
#             msg = f'{prefix}SPIKE ⚡ 时空数据延迟处理: {im_file}'
#             shape = (0, 0)  # 占位符，实际尺寸在transform阶段解析
#         except Exception as e:
#             nc = 1
#             msg = f'{prefix}ERROR ❌ 时空数据文件损坏: {im_file} ({e})'
#             return im_file, None, None, None, None, nm, nf, ne, nc, msg
#     else:
#         # 原有图像处理逻辑保持不变
#         try:
