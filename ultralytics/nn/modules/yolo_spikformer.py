...
class SpikeDFL(nn.Module):
    ...
    def __init__(self, c1=16):
        ...
        self.conv.weight.data[:] = nn.Parameter(x.view(1, c1, 1, 1))  # 这里是整数乘法
...
class SpikeDetect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(SpikeConv(x, c2, 3), SpikeConv(c2, c2, 3), SpikeConvWithoutBN(c2, 4 * self.reg_max, 1)) for x
            in ch)
        self.cv3 = nn.ModuleList(
            nn.Sequential(SpikeConv(x, c3, 3), SpikeConv(c3, c3, 3), SpikeConvWithoutBN(c3, self.nc, 1)) for x in ch)
...
class RepConv(nn.Module):
    def __init__(
        ...
        conv3x3 = nn.Sequential(
            # MultiStepLIFNode(detach_reset=True,v_reset=None, backend='torch'), #11111
...
class MS_GetT(nn.Module):
    ...
    def forward(self, x):
        if x.shape[0] == 1:  # 检查第一维是否为1
            x = x.squeeze(0)
        # 上一个网络输入是（T,B,C,H,W）
        if len(x.shape) == 4:
            x = (x.unsqueeze(0)).repeat(self.T, 1, 1, 1, 1)
        elif len(x.shape) == 5:
            x = x # 1,1,3,256,320 #CHW
        # print(f"模型内图像范围: 最大值={torch.max(x)}, 最小值={torch.min(x)}")
        return x
